    ...
    return (
        ...
        ${navStore.head && navStore.lastScroll > 30 ? 'top-[63px] md:top-[79px]' : 'top-[-1px]' } border-b-items hidden sm:block`}>
            <div className={`${pathname.startsWith('/menage/') ? "" : "xl:w-[90%]"} w-full h-full flex-y-center gap-4`}>
                <Link href={`/menage/${param}`} className="py-1 px-2 hover:text-blue-400 h-full transition-300 relative flex-center">
                    ภาพรวม
                    <span className="absolute w-full h-1 bg-blue-400 rounded-t-full -bottom-[1px] left-0"></span>
                </Link>
                <Link href={`/menage/${param}/edit`} className="py-1 px-2 hover:text-blue-400 h-full transition-300 relative flex-center">
                    ข้อมูลทั่วไป
                </Link>
                <Link href={'#'} className="rounded-md py-1 px-2 hover:bg-blue-500/10 transition-300 relative flex-center">
                    สถานที่ใกล้กับหอพัก
                </Link>
                <Link href={'#'} className="rounded-md py-1 px-2 hover:bg-blue-500/10 transition-300 relative flex-center">
                    รีวิวหอพัก
