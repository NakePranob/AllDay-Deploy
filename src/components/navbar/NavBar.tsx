...
const NavBar = observer(() => {
    ...
    const DrawerList = (
        <Box sx={{ width: 300, height: '100vh' }} role="presentation"
        onClick={() => setOpen(false)} className='bg-base flex flex-col'>
            <List className='flex px-4 pb-4 pt-4 border-b-items'>
                ...
                <div className="container flex-y-center justify-between">
                    ...
                    <div className='flex-y-center hidden sm:flex'>
                        <Tooltip title="เปิดเมณู" onClick={()=>setOpen(true)}>
                            <h1 className={`cursor-pointer text-3xl font-bold text-black dark:text-white relative me-6 ${inter.className}`}>
                                <TbMapPinSearch className="absolute text-xl -right-4 -bottom-1 text-blue-400"/>
                                ALLDAY
                                <span className="h-[.2rem] w-20 bg-blue-400 rounded-full absolute right-2 -bottom-1"></span>
                            </h1>
