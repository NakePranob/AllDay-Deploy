...
class dormitoryOnly {
    ...
    async addReserve(e: React.FormEvent<HTMLFormElement>, userId: number | null | undefined, roomId: number | null | undefined) {
        ...
        if (userId && roomId) {
            ...
            try {
                ...
                const result = await axios.post(`https://localhost/api/reserve`, formData);
                this.setAlert({
                    open: true,
                    state: 'success',
                    text: 'จองสำเร็จ',
        ...
        try {
            if (dmtId && userId) {
                ...
                if (!existingFavorite) {
                    ...
                    const result = await axios.post(`https://localhost/api/favorite`, formData);
                    ...
                    // ลบ favorite
                    const result = await axios.delete(`https://localhost/api/favorite/${existingFavorite.id}`);
    ...
    async getUserState(id: number | null | undefined) {
        try {
            if (id) {
                const resultLiveat = await axios.get(`https://localhost/api/liveat/${id}`);
                const resultFavorite = await axios.get(`https://localhost/api/favorite/${id}`);
                runInAction(() => {
    ...
    async review(e: React.FormEvent<HTMLFormElement>, id: number | null | undefined) {
        ...
        try {
            if (id) {
                ...
                if (formData.rating !== '') {
                    const result = await axios.post(`https://localhost/api/dormitory/review`, formData);
                    runInAction(() => {
